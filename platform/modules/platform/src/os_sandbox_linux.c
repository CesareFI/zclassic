 ...
 * live here because platform/modules/platform is the blessed raw-OS layer. The Landlock
 ...
 * harness in tests/harness/src/test_confine.c — each syscall was added only after a
 ...
 * the -confine help text and tests/harness/src/test_confine.c. */
 ...
 * `grep -rln` over core/modules/net, engine/modules/rpc, app/controllers, engine/composition/src, src for
 ...
 * connect() after sr_confine_enter (engine/composition/src/boot.c). accept4() is NOT
...
static const int g_node_confine_serving_allowed[] = {
     ...
     * it (grep-verified against core/modules/net + engine/modules/rpc + app/controllers). */
    ...
    __NR_connect,    /* connman.c reactor (outbound peer dial), nat.c
                       ...
                       * (ROM peer fetch), engine/modules/rpc/legacy_rpc_client.c,
                       * engine/modules/rpc/client.c */
#ifdef __NR_send
    ...
    __NR_sendto,     /* net.c peer writer, nat.c (UDP UPnP/STUN datagrams),
                       ...
                       * engine/modules/rpc/legacy_rpc_client.c,
                       * engine/controllers/rpc_client.c — the raw syscall
    ...
    __NR_setsockopt, /* netbase.c, nat.c, rom_fetch.c, https_server.c,
                       * engine/modules/rpc/legacy_rpc_client.c, engine/modules/rpc/client.c,
                       * engine/modules/rpc/httpserver.c, engine/controllers/api_controller.c,
                       * engine/controllers/explorer_controller.c,
                       * engine/controllers/wallet_view_helpers.c, net.c —
                       * SO_REUSEADDR / TCP_NODELAY / socket timeouts */
    __NR_shutdown,   /* https_server.c, engine/modules/rpc/rpc_timeout.c,
                       * engine/modules/rpc/httpserver.c, file_service.c — half-close
    ...
    const struct os_sandbox_path_rule *fs_rules, size_t n_fs_rules)
{
     ...
     * SERVICES_RUNNING boundary via a SYSINIT record in engine/composition/src/boot.c. */
    size_t n_denied = 0;
    ...
    const struct os_sandbox_path_rule *fs_rules, size_t n_fs_rules)
{
     ...
     * engine/composition/src/boot.c (sr_sandbox_enter, -confine branch). */
    size_t n_allowed = 0;
    ...
    const struct os_sandbox_path_rule *fs_rules, size_t n_fs_rules)
{
     ...
     * engine/composition/src/boot.c (sr_confine_enter, -confine=serving branch). */
    size_t n_allowed = 0;
    const int *allowed =
