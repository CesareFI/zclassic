 ...
 * sync_monitor_tip_advance_age() via the engine/modules/metrics tick.
 ...
 * the REJECT_IF/UNLESS macros in core/modules/validation/src/check_*.c.  Beyond
                                ...
                                int64_t zcl23_count)
{
    ...
    /* Last of the gauge setters engine/modules/metrics's 1-second thread calls each
     * tick (set_node_gauges, set_sync_state, set_header_gap,
     * set_tip_advance_age, set_mirror_lag, set_peer_kinds — see
     * engine/modules/metrics/src/metrics.c's metrics_thread_fn) — so by this point
 ...
 * use (engine/modules/framework/src/condition.c) — with a `metric_alert.` name
...
void metrics_prometheus_set_sync_state(int state, const char *name)
{
     ...
     * by metrics_prometheus_set_node_gauges, which engine/modules/metrics's tick always
...
size_t metrics_prometheus_render_prometheus(char *buf, size_t cap)
{
     ...
     * cardinality — see engine/modules/metrics/src/stage_metrics.c. */
