 ...
 * (engine/services/src/catchup_lifecycle_service.c); these three stay as thin
    ...
    struct boot_svc_ctx *svc, const struct chain_params *params)
{
    ...
    if (!supervisor_start()) {
        fprintf(stderr,  // obs-ok:supervisor-start-fallback-warn
            "WARNING: supervisor_start failed; engine/modules/health sweeper alone\n");
    }
                        ...
                        struct boot_svc_ctx *svc)
{
     ...
     * the stall). The tracked worker (core/modules/net/src/peer_strategy_worker.c)
