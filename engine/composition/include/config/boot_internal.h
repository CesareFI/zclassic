 ...
 * Not for use outside engine/composition/src/. */
 ...
 * (boot_block_file_scan.c). Shared across those two engine/composition/src TUs. */
...
struct boot_svc_ctx {
     ...
     * (core/modules/net/src/peer_strategy_worker.c). Spawned where the synchronous
 ...
 * (engine/composition/src/boot_pv_lookahead.c). start is a no-op unless ctx->pv_lookahead
 ...
 * The background-worker unit (engine/composition/src/boot_background_workers.c) was lifted
 ...
 * sites there). Declared here so the two engine/composition/src TUs share one definition. */
 ...
 * so the restart regression test (tests/harness/src/test_rpc_service_restart.c)
 ...
 * engine/services/src/catchup_lifecycle_service.c (catchup_lifecycle_start/
...
bool boot_sd_watchdog_test_runtime_pillars(bool sweep_alive,
                                           bool tick_alive,
                                           bool connman_alive);
/* One runtime pillar: its loop marker is fresh, the kernel reports work, or
 * an explicit operation-specific bounded wait remains inside its deadline. */
 ...
 * live in platform/modules/util/src/supervisor_backstop.c; this unit is only the
 ...
 * verdict.md): registers platform/modules/util/mem_pressure's poll on the SAME health
...
/* ── engine/composition/src/boot_anchor_snapshot_reachability.c ─────────────────────── *
...
/* ── engine/composition/src/boot_regtest_shielded.c ─────────────────────────────────
