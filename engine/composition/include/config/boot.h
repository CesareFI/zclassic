...
struct app_context {
     ...
     * (engine/composition/src/boot_bundle_fetch.c) as the file-service seed set when
...
#define APP_CONNECT_PEERS_MAX 8
    ...
    bool sandbox_steady;       /* -sandbox=steady : enter the os_sandbox node
                                 ...
                                 * the off-systemd self-respawn (engine/entry/main.c) would
...
#define APP_HTTPS_ALT_DOMAINS_MAX 8
    ...
    bool cold_start_seed_oneshot; /* -coldstart-seed-oneshot : INTERNAL cold-start
                                 * driver handshake (engine/composition/src/boot_cold_start.c).
    ...
    bool prefetch_blocks;      /* -prefetch-blocks : start the bounded, supervised
                                 * block-body read-ahead worker (engine/modules/storage/
    ...
    bool pv_lookahead;         /* -pv-lookahead[=0] : start the bounded, supervised
                                 ...
                                 * pool (engine/jobs/pv_lookahead.c) alongside the
 ...
 *
 * wallet_at_rest_creation_policy() (contexts/wallet/modules/wallet) is the pure, env-driven
...
/* -refold-from-anchor (B2; impl in engine/composition/src/boot_refold_staged.c): FULL-resets
 ...
 * (engine/composition/src/boot_refold_staged.c) in this order: (1) $ZCL_MINT_ANCHOR_OUT
...
/* -load-snapshot-at-own-height=PATH (impl in engine/composition/src/boot_refold_staged.c):
...
/* D1 refusal (engine/composition/src/boot_seed_gate.c). Peek the snapshot header (cheap, no
...
/* D3 preflight (engine/composition/src/boot_seed_gate.c). For the seed one-shot
...
/* -install-consensus-bundle=PATH (impl in engine/composition/src/boot_install_consensus_bundle.c):
...
/* -verify-consensus-bundle=PATH (impl in engine/composition/src/boot_verify_consensus_bundle.c):
 ...
 * engine/composition/src/boot_ratify_mint_anchor.c): TERMINAL — NEVER returns; it _exit()s
...
/* -verify-rom=(no arg, acts on -datadir) (impl in engine/composition/src/boot_verify_rom.c):
 ...
 * engine/composition/src/boot_export_consensus_bundle.c): TERMINAL — NEVER returns; it
 ...
 * impl in engine/composition/src/boot_promote_shielded_history.c): TERMINAL — NEVER returns;
 ...
 * icb_reset_utxo_mirror in engine/composition/src/consensus_state_install_derived_state.c): wipes the
...
/* -mint-anchor (impl in engine/composition/src/boot_refold_staged.c): the ANCHOR-SET MINT
...
/* -mint-anchor driver (impl in engine/composition/src/boot_mint_anchor.c): after app_init
 ...
 * engine/composition/src/boot_mint_anchor_rom_keystone.c — separate file for the E1
...
bool boot_mint_anchor_should_log_progress(int32_t applied_through,
                                          int32_t anchor);
/* mint-progress.log telemetry (impl in engine/composition/src/boot_mint_anchor_log.c —
...
void boot_mint_anchor_progress_log_tick(const char *path, int32_t through,
                                        ...
                                        bool force);
/* Fail-closed mint-fold stall diagnosis (impl in engine/composition/src/boot_mint_anchor.c):
 ...
 * engine/composition/src/boot_mint_anchor_log.c. Not called by the mint driver itself
 ...
 * engine/composition/src/boot_mint_anchor_preflight.c). PROBLEM this replaces: a fresh
...
/* B2 1c — boot torn-import AUTO-ARM (impl in engine/composition/src/boot_refold_staged.c).
 ...
 * site (engine/composition/src/boot_services.c): a normal boot of a TORN datadir self-heals,
 ...
 * engine/composition/src/boot_refold_staged.c). PURE precondition for the from-anchor fold:
...
/* -load-verify-boot eligibility probe (impl in engine/composition/src/boot_refold_staged.c).
...
/* Pure predicate (impl in engine/composition/src/boot_legacy_import.c):
...
/* Pure predicate (impl in engine/composition/src/boot_legacy_import.c): decide whether
...
/* Dispatch wrapper (impl in engine/composition/src/boot_legacy_import.c): runs
 ...
 * to keep engine/composition/src/boot.c under the E1 file-size ceiling. */
