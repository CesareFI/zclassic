...
This file defines public-node V1 acceptance (C1–C8) only. 8/8 is necessary
for platform v1 but does not establish it. The full C23 platform journey, its
release demonstration and the ordered work queue live in
[`work/FORWARD_PLAN.md`](./work/FORWARD_PLAN.md); durable priorities and
authority boundaries live in [`../AGENTS.md`](../AGENTS.md).
...
| 3 | **Cold-start sync to tip in <10 min** | Fresh datadir → `z23 core sync status` reports `phase=ready` within 10 min on 100 Mbps link — test: `tests/harness/src/test_cold_start_sync.c` (**CI slice: `make ci-mvp-gates`** — drives the sync FSM to `at_tip` in ~7s; does **not** download/validate a real 3M-block chain). The assisted-seed sub-proof is `make mvp-coldstart-local`: fixture-gated operator bundle (`block_index.bin` + `utxo-seed-*.snapshot`), fresh `/tmp` datadir, `-load-snapshot-at-own-height`, and >1M UTXOs body-digest verified and installed in <90s (`make mvp-coldstart-local` passes the bundle seed-integrity proof in ~17s: `count=1344903`, body SHA3 OK). That proves byte integrity and operational seeding, not sovereign state provenance. The full C3 proof command is `make mvp-coldstart-to-tip-local`: same bundle, fresh datadir, dial a serving z23 peer, and reach that peer's captured tip within the 10-minute MVP budget. Legacy `consensus_snapshot.db` import remains only a checkpoint-height fallback; above-checkpoint peer snapshots are intentionally refused because there is no in-binary root. A snapshot-only node dialing a live serving peer does not reach `at_tip` on its own, because the snapshot import path sets `coins_best_block` without seeding `coins_kv` / `coins_applied_height`; the full tip-probe harness uses the bundle path to close that gap. The genuinely-wiped counterpart (no bundle, no snapshot flag at all — just the binary's own boot pipeline, `tools/scripts/cold_start_to_tip_stopwatch.sh` / `make mvp-coldstart-to-tip-stopwatch`) is the harness that [`FORWARD_PLAN.md`](./work/FORWARD_PLAN.md) §5 item 1's production wiring builds on: it wipes a `/tmp` datadir, dials a real serving peer, and gates on `dumpstate reducer_frontier`'s `hstar` reaching `network_tip` — never on "the FSM says at_tip" — printing a real `WALL_CLOCK_SECONDS=<n>` stopwatch on PASS (`ZCL_BIN=`/`ZCL_PEER=` override which binary/peer it times; SKIPs cleanly with no peer). **Nothing outside prose records any of those 2026-09-09 runs**: the collector ledger (`~/.local/state/zclassic23-c3-stopwatch/history.jsonl`) holds no row of any verdict for 2026-09-09, and no `build/c3-stopwatch/20260909T*` artifact directory exists on disk. This cell used to claim the harness ran end-to-end on this host three times on 2026-09-09 (P2P peers 127.0.0.1:8033 + devfleet + soak, file service 127.0.0.1:18034), verdict SEAM every time — the ROM fold covering 0→~3.09M in ~150 s, then a ~150k-block tail said to crawl at ~98 received bodies/s with a ~12% request-timeout rate on loopback against a ~420/s sustained requirement — and that a PASS followed later the same day at 336 s, reproduced at 371 s, against a dedicated fixture peer, written into this cell by commit `c3c485d5d3`. Both the SEAM runs and the PASS are **withdrawn here** on the same ground: no ledger row, no artifact directory, nothing outside prose backs either; the 2026-09-09 blocks in [`work/fresh-start-seam.md`](./work/fresh-start-seam.md) §0 and the timing note in [`work/stopwatch-gates.md`](./work/stopwatch-gates.md) are marked withdrawn for it too. What the ledger does say is the opposite. Its newest row is 2026-09-08 20:08 UTC, `verdict=fail`, `no_pass_streak=77`; its newest `verdict=pass` is 2026-08-21 at `wall_clock_seconds=515` against `final_network_tip=3224124` (`final_hstar=3224132`); 14 of its 240 recorded runs passed. That 515 s pass is real evidence that the wipe path can reach network tip inside the 600 s budget, but it is three weeks stale, so it does not carry the current claim. The "complete shielded-history gates" condition is satisfied by construction on this path: a genuinely wiped datadir performs no import, and the reducer's fail-closed shielded-completeness holds (utxo_apply holding Sapling/Sprout spends under incomplete history) would have stopped the fold — they did not fire in the 2026-08-21 pass, whose `final_hstar=3224132` cleared `final_network_tip=3224124`. **2026-09-11 495 s PASS — UNVERIFIED.** Commit `00a76c7847` recorded a wipe-to-tip PASS (`hstar=3246606` ≥ `network_tip=3246599`, `WALL_CLOCK_SECONDS=495` of 600 across 2 boots, artifact `build/c3-stopwatch/20260911T063020Z-401254`). Checked later that day: the default ledger on the maintainer host holds no row for 2026-09-11, and the named artifact is absent from that host's checkouts and lanes and from the macOS host; the producing host is not otherwise recorded. The claim stays in this record as unverified, and no later PASS can substantiate it. **2026-09-11 21:24:40Z run — ENVIRONMENT_BLOCKED, not a sync measurement:** candidate `72799dcf76` (executable sha256 `6cf665a49ad3c913dff42371347d15aff3ba9aa03eb6a5e0eb38e9e289607b15`, source id `bb419ca35a18f5e915bb854188bad881f7954356a4bb7b85951ce32f4b09ee9f`), one host, loopback fixture peer (P2P plus file service), no bundle or header seed, 1 boot, 605 s. The candidate dialed the fixture and sent `version`; the fixture never read it (its inbound handshake timed out at `version=0`) and its own tip had not advanced since its 2026-09-08 restart, so the run ended `final_hstar=0` against `final_network_tip=-1` (no handshake-complete peer), ledger `verdict=stalled-named`. That observes a broken fixture; repair the fixture before the next run. Reaching tip and completing full-history validation remain separate claims. **2026-09-11 22:16:56Z run — PRODUCT_FAILURE, measured against a repaired fixture:** candidate `0321d315e3` (executable sha256 `7e33438ed4f8bcfb6bae656ef8afff31278aef65db48806060c54a22d6b8aa22`, source id `3ce56ca35ca21d66fa5df47b8377a10e36a7fe84a93f50ec19135d2d50be961f`), one host, loopback fixture peer (P2P 39070 plus file service 39072), no bundle or header seed, 1 boot, 609 s against a 600 s budget. The fixture was restarted at 21:58:04Z because its connection-manager message thread was wedged (declared in the intervention ledger); it held 5 active peers at its own frontier (cursor 3247339 of 3247340) before and after this run. The candidate completed the handshake in 10 s and read `network_tip=3247339`, but `hstar` never left 1: the headers, bodies and fold phases all started and none ended, with named blockers `worker.projection_stuck`, `address_index.below_snapshot_seed`, `txindex.below_snapshot_seed`, `bundle_exporter.degraded`, `sync_rate_below_floor`, `sticky_escalator.resnapshot_no_base` and `sticky_escalator.refold_no_anchor_artifact`; ledger row ts `1789165639`, artifact `build/c3-stopwatch/20260911T221656Z-2130298`, and `make mvp` reports C3 FAIL from that row. A genuinely wiped node with a healthy peer at tip therefore folds nothing: that is a substantiated product blocker, and it is a new observation that neither substantiates nor refutes the unverified 495 s claim. The 2026-09-09 claims stay withdrawn. A durable evidence ledger for both this stopwatch and its network-disruption-recovery sibling (PROOF B) — periodic collect/judge split, `make c3-stopwatch-report` / `make netdisrupt-stopwatch-report` — is documented in [`work/stopwatch-gates.md`](./work/stopwatch-gates.md). | ✅ |
...
+ `make test-two-node-peer-tip`, …). The MRS counts only ✅. A ✅ records a historically demonstrated run. It is not
current-candidate qualification: `make mvp` reports that from fresh receipts
bound to the exact running executable, and a missing, stale, mismatched or
incomplete receipt reports BLOCKED, never PASS.

**MRS = the count of ✅ criteria above** (8/8 = MVP achieved). Current MRS:
`z23 milestone` (REST `GET /api/v1/milestone`). Neither this file nor
[`docs/HANDOFF.md`](./HANDOFF.md) tracks a live count.
...
`test-crash-bootstrap` + `test-two-node-peer-tip` (kill-9 recovery) — and C3
via the wipe-to-tip stopwatch (recorded 515 s PASS on 2026-08-21; the 2026-09-11 495 s claim is unverified). **C3 qualifies again as of 2026-09-14:** candidate `d571f466c8` reached the captured fixture tip (hstar 3,250,259 ≥ network_tip 3,250,253) in **561 s** across 2 boots (artifact `build/c3-stopwatch/20260914T104211Z-3243980`, ledger `verdict=pass`). The 24-day `hstar=1` streak was root-caused to a stale pre-auth fixture peer binary (Aug-22, predating the Aug-23 handshake commit `d1db47aee7`), not product sync logic; the fixture was rebuilt and a file-service handshake preflight (`3de35c2a62`) now fails fast on fixture incompatibility. A qualifying C3 run on the final soak candidate `129bc03fa3` completed 2026-09-15: hstar 3,251,226 ≥ network_tip 3,251,217 in **431 s** across 2 boots (artifact `build/c3-stopwatch/20260915T062816Z-2530929`, binary sha256 `5353fd09…`, ledger `verdict=pass`). The same day's first attempt was environment-blocked — the serving fixture peer wedged its inbound handshake path after 4h40m uptime (accepted TCP, never sent its version; a fixture restart restored service, wedge evidence preserved for a soak-class connman follow-up) — not product sync logic. Remaining
◐ criteria: **C6** (needs a clean 168h
...
**C5 closed 2026-09-14 on `fc42ddd333`:** the owner store-dispatch decision
(Option 2: complete the onion dynhost POST path) landed — `dynhost_client_fetch_ex`
(Tor vendor `f4e28103c`, child of published pin `92687dc865`),
`tor_integration_fetch_onion_post(+_blocking)`, the `app.store.remotebuy`
leaf, and the v83 `seller_onion` purchase schema — and the two-node
public-Tor acceptance (`tools/dev/store_onion_acceptance.sh`) PASSed the full
journey over real circuits: seller lists a file product, buyer discovers +
...
**Ordering:** open ◐ criteria are P1 work under [`../AGENTS.md`](../AGENTS.md).
[`docs/work/FORWARD_PLAN.md`](./work/FORWARD_PLAN.md) owns the ordered queue.
Each ◐ cell above names its own remaining condition or owner decision.
