...
| `fixture_incompatible` | **1** | never self-heals — the fixture ANSWERED but cannot serve this node (e.g. its binary predates the authenticated X25519/HKDF file-service handshake, d1db47aee7), so the identical skip recurs until the fixture is rebuilt. Measured 2026-09: that shape passed a bare TCP-connect precheck and burned the full 600s C3 budget per run for ~3 weeks | `file_peer fixture incompatible: …` |
