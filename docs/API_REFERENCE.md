...
| `app tasks` | ready | mutate / app-write / operator · background/low | **`datadir`**, `app`, **`action`**, `expected_revision`, `task_id`, `title` | `zcl.task_document.v1` | `z23 app tasks open /absolute/private/tasks` | Save and reopen your local tasks |
...
| `app presentation ecosystem` | ready | mutate / app-write / operator, display-only · foreground/moderate | `output` | `zcl.app_presentation_ecosystem.v1` | `z23 app presentation ecosystem` | Show the exact Z23 C23 ecosystem in one native window |
...
| Command | Avail | Policy | Input keys (**required**) | Output schema | Example | Summary |
|---|---|---|---|---|---|---|
| `dev retrieval benchmark` | compat 🔧 → `z23-dev dev retrieval benchmark` | read / read / operator · persistent/high | `workspace`, `expected_vcs_root`, `task_id`, `query`, `cursor` | `zcl.dev_retrieval_benchmark.v3` | `z23-dev dev retrieval benchmark --input='{"workspace":"<canonical-absolute-checkout>","expected_vcs_root":"<64-lowercase-hex>","task_id":"task","query":"goal"}'` | Compare literal, BM25, and identifier-graph file retrieval — *retrieval benchmarking requires the dev binary* |
| `dev retrieval experiment` | compat 🔧 → `z23-dev dev retrieval experiment` | read / read / operator · persistent/high | `workspace`, `expected_vcs_root`, `task_id`, `query`, `profile_hex`, `study_root`, `preregistration_root`, `evaluator_root` | `zcl.dev_retrieval_experiment.v3` | `z23-dev dev retrieval experiment --input='{"workspace":"<canonical-absolute-checkout>","expected_vcs_root":"<64-lowercase-hex>","task_id":"task","query":"goal","profile_hex":"<112-lowercase-hex>","study_root":"<64-lowercase-hex>","preregistration_root":"<64-lowercase-hex>","evaluator_root":"<64-lowercase-hex>"}'` | Project one immutable direct-gold-input-free retrieval profile — *retrieval experiments require the dev binary* |
...
| `dev proof step` | compat 🔧 → `z23-dev dev proof status` | mutate / dev-mutation / **owner** · foreground/high | `root`, **`local_commit`**, **`remote_base`** | `zcl.dev_proof_status.v1` | `z23-dev dev proof step --local_commit=<commit> --remote_base=<base>` | Execute one explicit proof pair in the foreground without a watcher — *foreground proof requires the dev binary* |
...
| `zcode work map` | ready | read / read / operator · fast/low | `workspace`, **`map_root`**, `offset`, `limit`, `proof_datadir`, `action_id`, `node_index`, `accepted_work_root` | `zcl.zcode_work_map.v1` | `z23 zcode work map --input='{"map_root":"<sha3-root>","limit":4}'` | Inspect one exact work-map hierarchy |
...
| Command | Avail | Policy | Input keys (**required**) | Output schema | Example | Summary |
|---|---|---|---|---|---|---|
| `zcode package guide` | ready | read / read / public · instant/tiny | none | `zcl.zcode_package_guide.v1` | `z23 zcode package guide` | Exact package workflow and authority boundaries |
...
| `zcode package show` | ready | read / read / operator · fast/low | **`root`**, `datadir` | `zcl.zcode_package_show.v1` | `z23 zcode package show --input='{"root":"<64hex>"}'` | Release record and manifest for one package root |
...
| `zcode package rollback` | ready | mutate / app-write / operator · fast/low | `name`, `now_unix`, `datadir` | `zcl.zcode_package_rollback.v1` | `z23 zcode package rollback --input='{}'` | Go back one step: re-activate the previous generation |
...
| Command | Avail | Policy | Input keys (**required**) | Output schema | Example | Summary |
|---|---|---|---|---|---|---|
| `zcode science work plan` | ready | mutate / app-write / operator, plan-commit · foreground/moderate | **`wire_hex`**, `method_hex`, `profile_hex`, `action_kind`, `action_sequence`, `action_source_cas_sha3`, `action_input_root_sha3`, `action_toolchain_capsule_sha3`, `now_unix`, `datadir`, `workspace` | `zcl.zcode_science_plan.v1` | `z23 zcode.science.work.plan --input='{"wire_hex":"<726hex>","method_hex":"<242hex>","profile_hex":"<444hex>","action_kind":1,"action_sequence":1,"action_source_cas_sha3":"<64hex>","action_input_root_sha3":"<64hex>","action_toolchain_capsule_sha3":"<64hex>"}'` | Plan benchmark evidence |
| `zcode science work commit` | ready | mutate / app-write / operator, plan-commit · foreground/moderate | **`wire_hex`**, **`confirm`**, `action_kind`, `action_sequence`, `action_source_cas_sha3`, `action_input_root_sha3`, `action_toolchain_capsule_sha3`, `now_unix`, `datadir`, `workspace` | `zcl.zcode_science_commit.v1` | `z23 zcode.science.work.commit --input='{"wire_hex":"<726hex>","confirm":true,"action_kind":1,"action_sequence":1,"action_source_cas_sha3":"<64hex>","action_input_root_sha3":"<64hex>","action_toolchain_capsule_sha3":"<64hex>"}'` | Commit planned benchmark evidence |
