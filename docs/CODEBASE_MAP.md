...
1. Declare the command in the matching `engine/composition/commands/*.def` bundle.
   There are 31 command bundles; `ls engine/composition/commands/*.def
...
| registry commands | `z23 discover help` — 13 command roots (`core`, `app`, `dev`, `ops`, `discover`, `code`, `vault`, `zcode`, `metaverse`, `yardsale`, `zses`, `story`, `fleet`) plus the bare `status` leaf, so 14 top-level names — then `discover help <path>` to descend |
...
| `build/bin/z23-dev dev proof ensure/status/wait` | Exact commit/advertised-base acceptance receipt lifecycle. A hook never arms a resident the user did not arm: post-commit/merge/checkout only re-arm a resident proof watcher where `<root>/.cache/zcl-dev-watch.lock` already shows one was started on purpose (`dev loop ensure` / `dev proof ensure`), so an untouched worktree stays quiet on every commit; push admission validates the fixed-width self-sealed receipt without building, testing, linting, waiting, or invoking a shell. The queue is still filesystem-backed and is not the unfinished signed-commit publisher. |
| `make git-hook-selftest` / `make install-hooks` | Builds, measures, and installs the native C23 Git hook in the checkout-local hook directory. `pre-push` admits receipts; post-commit/merge/checkout re-arm proof work only where a resident was already armed (`.cache/zcl-dev-watch.lock` present). |
