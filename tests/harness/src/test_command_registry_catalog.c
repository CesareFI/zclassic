...
static int test_retrieval_experiment_profile_input(void)
{
    int failures = 0;
    const struct zcl_command_registry *reg = zcl_command_catalog();
    TEST("retrieval experiment exposes only the immutable profile input") {
        const struct zcl_command_spec *spec =
            find_spec(reg, "dev.retrieval.experiment");
        ASSERT(spec != NULL);
        ASSERT(strcmp(spec->output_schema,
                      "zcl.dev_retrieval_experiment.v3") == 0);
        char why[160];
        struct json_value input;
        json_init(&input);
        json_set_object(&input);
        json_push_kv_str(&input, "profile_hex", "profile-wire");
        ASSERT(zcl_command_registry_input_validate(spec, &input, why,
                                                   sizeof(why)));
        json_free(&input);
...
static int test_zses_invite_leaves(void)
{
    ...
    TEST("zses invite create/accept and ops.mesh.join are discoverable") {
        ...
        const struct zcl_command_spec *pair_list =
    ...
    TEST("ops.mesh pair plan/commit are discoverable") {
        const struct zcl_command_spec *plan =
            find_spec(reg, "ops.mesh.pair.plan");
        const struct zcl_command_spec *commit =
            find_spec(reg, "ops.mesh.pair.commit");
        ASSERT(plan != NULL);
        ASSERT(commit != NULL);
...
/* The ZCL application-feature leaves (engine/composition/commands/app_features.def).
 *
 ...
 *            engine/controllers/src/app_write_native_handlers.c over a backing
...
/* engine/composition/src/command_catalog.c also builds a SECOND, parallel stringizing
