 ...
 * this file forks raw transports instead, so retain correctly typed
 * references to keep -Wunused-function quiet. Function pointers are not
 * object pointers in ISO C and need no cast when their exact types are kept. */
static bool (*const mr_spawn_fake_ref)(
    enum fake_mode, int, const struct muse_session_limits *,
    struct fake_host *) = spawn_fake;
static void (*const mr_close_fake_ref)(struct fake_host *) = close_fake;
...
{
    int failures = 0;
    (void)mr_spawn_fake_ref;
    (void)mr_close_fake_ref;
