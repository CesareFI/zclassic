 ...
 * engine/composition/src/boot_services.c (core liveness registers before the frontend,
...
int t_lib_runtime_gauges_are_callback_injected(void)
{
    ...
    TEST("lib runtime gauges and peer preference are callback injected") {
        ...
        ASSERT(repo_path(path, sizeof(path), "engine/modules/metrics/src/metrics.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/connman.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_hodl_history_uses_runtime_db_service(void)
{
    ...
    TEST("HODL history worker uses runtime DB service") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path),
                         "engine/composition/src/boot_background_workers.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_db_service_query_handle_is_canonical(void)
{
    ...
    TEST("DB service query handle aliases canonical node DB") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/db_service.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_txindex_releases_node_db_between_batches(void)
{
    ...
    TEST("txindex releases node.db between batches") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path),
                         "engine/controllers/src/snapshot_controller_txindex.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_peer_save_busy_reports_db_error(void)
{
    ...
    TEST("peer save lock exhaustion is reported as DB error") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path), "engine/models/src/peer.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_handshake_peer_save_is_async(void)
{
    ...
    TEST("handshake peer persistence is advisory async write") {
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "engine/composition/src/boot_msg_callbacks.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/msg_version.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/peer_lifecycle.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_tx_wallet_sync_is_callback_injected(void)
{
    ...
    TEST("tx wallet persistence and snapshot state are injected into net") {
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/msg_tx.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_p2p_block_submit_is_callback_injected(void)
{
    ...
    TEST("p2p block submission is injected into net") {
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/msg_blocks.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_flyclient_proof_builder_is_callback_injected(void)
{
    ...
    TEST("FlyClient proof builder is injected into net snapshot handler") {
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/boot_flyclient.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "boot_build_flyclient_proof") != NULL);
        ASSERT(strstr(buf, "snapsync_build_fc_response") != NULL);
        ASSERT(strstr(buf, "boot_load_block_hashes_range") != NULL);
        ...
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/net/src/msgprocessor_snapshot.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_fast_sync_uses_lib_sqlite_helpers(void)
{
    ...
    TEST("fast sync avoids direct AR, DB, and UTXO model includes") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/fast_sync.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/include/net/fast_sync.h") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/boot_snapshot_offer.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_framework_reexport_headers_stay_deleted(void)
{
    ...
    TEST("framework primitive re-export headers stay deleted") {
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "engine/modules/framework/include/framework/mailbox.h") == 0);
        errno = 0;
        ASSERT(stat(path, &st) != 0 && errno == ENOENT);

        ASSERT(repo_path(path, sizeof(path),
                         "engine/modules/framework/include/framework/projection.h") == 0);
        errno = 0;
        ASSERT(stat(path, &st) != 0 && errno == ENOENT);

        ASSERT(repo_path(path, sizeof(path),
                         "engine/services/include/services/header_admit_inbox.h") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "engine/controllers/src/chain_projection.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path), "engine/modules/framework/README.md") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_utxo_reimport_flag_is_storage_owned(void)
{
    ...
    TEST("utxo reimport flag is storage owned") {
        ...
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/boot.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "storage/utxo_reimport_flag.h") != NULL);
        ASSERT(strstr(buf, "utxo_reimport_flag_check_and_clear") != NULL);
        free(buf);
...
int t_net_sync_planners_are_lib_owned(void)
{
    ...
    TEST("net sync planners use lib-owned contract") {
        char path[PATH_MAX];
        ASSERT(repo_path(path, sizeof(path), "core/modules/net/src/msg_headers.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "sync/sync_planner.h") != NULL);
        ASSERT(strstr(buf, "msg_processor_snapshot_active") != NULL);
        ...
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/sync/include/sync/sync_planner.h") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        free(buf);
        buf = NULL;
        ASSERT(repo_path(path, sizeof(path), "engine/modules/event/include/event/event.h") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
...
int t_process_block_node_db_access_is_runtime_owned(void)
{
    ...
    TEST("process block node_db access is runtime owned") {
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_flush_policy.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "node_db_runtime_handle_open") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_self_heal.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_core.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "controllers/blockchain_controller.h") == NULL);
        ASSERT(strstr(buf, "controllers/sync_controller.h") == NULL);
        ASSERT(strstr(buf, "models/database.h") == NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_contextual_header.c")
               == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "process_block_should_skip_contextual_header")
               != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_tip_child.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "process_block_verify_active_tip_child_on_disk")
               != NULL);
        ASSERT(strstr(buf, "find_best_active_tip_child") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_tip_publish.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "process_block_commit_tip") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_index.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "find_block_pos") != NULL);
        ASSERT(strstr(buf, "block_index_refresh_header") != NULL);
        ASSERT(strstr(buf, "block_index_hydrate_from_disk") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_runtime_hooks.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "process_block_set_gap_fill_kick") != NULL);
        ASSERT(strstr(buf, "process_block_kick_gap_fill") != NULL);
        ASSERT(strstr(buf, "process_block_set_tip_publication_hooks") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path),
                         "core/modules/validation/src/process_block_failed_child.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ASSERT(strstr(buf, "process_block_propagate_failed_child") != NULL);
        ...
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/runtime.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
        ...
        ASSERT(repo_path(path, sizeof(path), "engine/composition/src/boot_tip_hooks.c") == 0);
        ASSERT(read_entire_file(path, &buf) == 0);
