 ...
 * level's path is hashed verbatim, so "/core/modules/net" can never reproduce the digest
 * minted for "core/modules/net" — but the TOPMOST dirname() collapse is invisible to the
...
/* The one byte-order codec (platform/modules/base/include/base/serialize_le.h), not a
