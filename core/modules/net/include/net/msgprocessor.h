 ...
 * peer? Answered by engine/services/market_moderation_service, which core/modules/net
 ...
 * core/modules/net must not name contexts/market/modules/zswap symbols (module-order ratchet), so the
...
struct msg_processor {
    ...
    /* ZCODE package swarm (slice 12): the engine lives ABOVE core/modules/net in
     * the module order (contexts/commons/modules/vcs), so net speaks to it only through these
