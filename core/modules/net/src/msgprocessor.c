...
/* core/modules/net still reaches the controller-owned manifest cache for P2P file
...
/* Supervisor liveness (root child — core/modules/net cannot include the app-side
                              ...
                              struct byte_stream *s)
{
     ...
     * reached through the injected ingest port (module-order seam: core/modules/net
     * never names a contexts/market/modules/zswap symbol). An invalid/expired ad is dropped
...
static const struct msg_dispatch_entry g_msg_dispatch[] = {
    ...
    /* ── zk proving-parameter transfer (core/modules/net/src/params_service.c) ──
...
bool msg_send_messages(void *ctx, struct p2p_node *node, bool send_trickle)
{
    ...
    /* ── IBD progress log (every 30s, first outbound peer only) ── */
    {
        ...
        if ((is_progress_peer || is_watchdog_peer) &&
            ...
            /* Sync watchdog now runs on the engine/modules/health periodic ring
