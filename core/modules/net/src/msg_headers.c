 ...
 * (engine/modules/storage/src/block_index_db.c: "Don't store solution in
 ...
 * (struct block_index_flat, engine/services/src/block_index_loader.c) persists no
 ...
 *
 *   engine/services/src/block_index_blocks_hydrate.c  — full_check is
 *     (h % BLOCKS_HYDRATE_POW_STRIDE == 0) || h > rom_checkpoint,
 *     i.e. one row in 10,000 below the ROM checkpoint;
 *   engine/services/src/block_index_loader.c          — calls
 ...
 *   engine/composition/src/boot_block_file_scan.c              — assigns
 *     BLOCK_VALID_TREE unconditionally;
 *   engine/composition/src/boot_header_seed_import.c           — CLAMPS a
 ...
 * (engine/jobs/src/validate_headers_validator.c, block_row_verify with
 ...
 * tests/harness/src/test_getheaders_serve_fallback.c case 7,
 * tests/harness/src/test_getheaders_serve_pow_dedup.c and
 * tests/harness/src/test_getheaders_serve_receipt.c. */
                                       ...
                                       struct block_header *hdr_out)
{
     ...
     * true return. The port keeps core/modules/net below app/jobs and config while
                        ...
                        struct byte_stream *s)
{
    ...
    if (node->getheaders_rate_window_count >= allowance) {
        ...
        if (getheaders_suppress_rising_edge(&g_getheaders_rate_streak))
            LOG_WARN("headers",
                     ...
                     "(deferred_rate_window=%llu allowance=%u parked=%u "
                     "replay_in=%llds)",
    ...
    /* Serving supersedes the parked ask — see getheaders_park_deferred(). */
                     ...
                     struct byte_stream *s)
{
...

    {
        ...
        msg_header_range_note_response(node, &header_plan.batch,
                                       accepted, count);
        ...
        if (syncsvc_should_restart_headers_from_tip(
                    ...
                    active_chain_height(&mp->main_state->chain_active));
            {
                ...
                if (restart_tip && restart_tip->phashBlock)
                    msg_push_getheaders_followup(mp, node, restart_tip,
                                                 follow_height);
                else
            ...
            msg_push_getheaders_followup(mp, node,
                                         mp->main_state->pindex_best_header,
                                         follow_height);
        } else {
            ...
            msg_push_getheaders_followup(mp, node, pindex_last, follow_height);
        }
    }
