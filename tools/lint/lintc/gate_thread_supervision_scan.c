...
static int tss_unreadable(const char *path, int error)
{
    fprintf(stderr, "%s: UNPROVEN — %s exists but cannot be\n"
            ...
            k_tss_name, path, strerror(error));
    return 2;
}
 ...
 * shell's fail-open. A mode with no read bits is refused before fopen so
 * the same source tree has the same verdict under root and an unprivileged
 * proof runner. Any other open failure (e.g. a TOCTOU removal
...
static int tss_scan_file(const char *path)
{
    ...
    if (stat(path, &st) == 0 &&
        ...
        return tss_unreadable(path, EACCES);

    FILE *f = fopen(path, "r");
    if (!f) {
        int e = errno;
        if (e == EACCES)
            return tss_unreadable(path, e);
